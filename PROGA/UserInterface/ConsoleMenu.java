...
import PROGA.Controller.Counter;
import PROGA.Exceptions.UncorrectDataException;
...
public class ConsoleMenu {
            ...
            while (flag) {
                ...
                switch (key) {
                    case "1":
                        ...
                        break;
                    case "2":
                        HomeAnimalsType type = menuChoice(in);
                        if (type != null) {
                            try {
    ...
    private HomeAnimalsType menuChoice(Scanner in) {
    ...
    private void menuTrainAnimal(int homeId, Scanner in) {
        Scanner sc = in;
        while (true) {
            ...
            if (command.length() == 1 && command.equals("0"))
                return;
            if (animalController.trainAnimal(homeId, command))
